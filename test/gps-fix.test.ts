...
import {
  CURRENT_FIX_MAX_AGE_MS,
  STALE_FIX_MAX_AGE_MS,
...
function createUtcDateWithOffset(secondsOffset = 0): Date {
  const date = new Date();
...
function buildRmcSentence(date: Date, status = "A"): string {
  return buildSentence(
    `GPRMC,${formatNmeaTime(date)},${status},4807.038,N,01131.000,E,022.4,084.4,${formatNmeaDate(date)},003.1,W`,
  );
}

function buildGgaSentence(date: Date, fixQuality = 1): string {
...
function buildGsaSentence(fixMode = "3"): string {
  return buildSentence(`GPGSA,A,${fixMode},04,05,09,12,24,25,29,31,,,,1.8,0.9,1.5`);
}

function assertApproximate(actual: number, expected: number, epsilon = 0.000001): void {
  assert.ok(
...
test("processNmeaSentence parses a valid RMC sentence into a fixed snapshot", () => {
  ...
  assertApproximate(result.snapshot?.metrics.latitude as number, 48.1173);
  assertApproximate(result.snapshot?.metrics.longitude as number, 11.516666666666667);
});
...
test("a no-fix GGA sentence does not downgrade a fresh valid RMC fix", () => {
  ...
  assertApproximate(afterNoFixGga.snapshot?.metrics.latitude as number, 48.1173);
  assertApproximate(afterNoFixGga.snapshot?.metrics.longitude as number, 11.516666666666667);
});
...
test("a recent last-known fix becomes stale after live sentence freshness expires", () => {
  ...
  const staleSnapshot = createMergedFixSnapshot(
    afterGga.fixState,
    lastGoodFix.tsMs + CURRENT_FIX_MAX_AGE_MS + 1000,
  );
...
test("an old last-known fix eventually returns to searching state", () => {
  ...
  const expiredSnapshot = createMergedFixSnapshot(
    afterGga.fixState,
    lastGoodFix.tsMs + STALE_FIX_MAX_AGE_MS + 1000,
  );
