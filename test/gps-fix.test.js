...
test('a recent last-known fix becomes stale after live sentence freshness expires', () => {
    ...
    const staleSnapshot = createMergedFixSnapshot(
        afterGga.fixState,
        afterGga.fixState.lastGoodFix.tsMs + CURRENT_FIX_MAX_AGE_MS + 1000
    )
    ...
    assert.ok(staleSnapshot.metrics.fix_age_ms > CURRENT_FIX_MAX_AGE_MS)
})
...
test('an old last-known fix eventually returns to searching state', () => {
    ...
    const expiredSnapshot = createMergedFixSnapshot(
        afterGga.fixState,
        afterGga.fixState.lastGoodFix.tsMs + STALE_FIX_MAX_AGE_MS + 1000
    )
    ...
    assert.equal(expiredSnapshot.fixDetails.last_valid_fix_at, afterGga.fixState.lastGoodFix.timestamp)
})
