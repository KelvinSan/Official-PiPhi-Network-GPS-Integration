...
export function toIsoTimestamp(value) {
    ...
    if (typeof value === 'string' && value.length > 0) {
        if (/^\d{2}:\d{2}:\d{2}(\.\d+)?$/.test(value)) {
            ...
            const [seconds, fraction = '0'] = secondsWithFraction.split('.')
...
export function deriveFixQuality(fixStatus, satellites, hdop, signalState) {
    ...
    if (Number.isFinite(hdop) && hdop <= 1.5 && Number.isFinite(satellites) && satellites >= 6) {
        return 'strong'
    }
    if (Number.isFinite(hdop) && hdop <= 3 && Number.isFinite(satellites) && satellites >= 4) {
        return 'good'
    }
    if (Number.isFinite(satellites) && satellites >= 3) {
        return 'usable'
    }
    return 'weak'
}
