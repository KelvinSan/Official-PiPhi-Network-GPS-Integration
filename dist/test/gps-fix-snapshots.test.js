...
import { applyParsedSentenceToFixState, createInitialFixState, createMergedFixSnapshot, processNmeaSentence, } from "../lib/gps-fix.js";
import { assertApproximate, buildGgaSentence, buildGsaSentence, buildRmcSentence, createUtcDateWithOffset, } from "./gps-fixture-helpers.js";
...
test("createMergedFixSnapshot uses fresh GGA coordinates when RMC is missing", () => {
    ...
    const result = applyParsedSentenceToFixState(createInitialFixState(), {
        sentenceId: "GGA",
...
test("applyParsedSentenceToFixState returns null snapshot for unsupported sentences", () => {
    const result = applyParsedSentenceToFixState(createInitialFixState(), { sentenceId: "GLL" }, Date.now());
...
test("applyParsedSentenceToFixState preserves previous lastGoodFix on unsupported sentences", () => {
    ...
    const afterUnknown = applyParsedSentenceToFixState(fixed.fixState, { sentenceId: "GLL" }, now.getTime() + 1000);
...
test("stale snapshots inherit last known coordinates", () => {
    ...
    assertApproximate(stale.metrics.latitude, 48.1173);
...
test("searching snapshots prefer fresh GSA hdop when GGA hdop is unavailable", () => {
    const now = Date.now();
    const afterGsa = applyParsedSentenceToFixState(createInitialFixState(), { sentenceId: "GSA", HDOP: 1.1, fixMode: "3" }, now);
...
test("lastGoodFix remains unchanged after searching snapshots", () => {
    const initial = createUtcDateWithOffset(0);
    const fixed = processNmeaSentence(createInitialFixState(), buildRmcSentence(initial), initial.getTime());
    const searching = applyParsedSentenceToFixState(fixed.fixState, { sentenceId: "GSA", fixMode: "0" }, initial.getTime() + 200000);
